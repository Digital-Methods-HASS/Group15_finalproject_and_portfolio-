...
Example: Just want to get text from a single page (e.g. Page 42)? 
```{r single-page}
got_p42 <- got_text[42]
got_p42
```

See how that compares to the text in the PDF on Page 42. What has pdftools added and where?
#pdftools has added line breaks (`\n`) to separate lines of text
...
### A word cloud of Game Of Thrones book words (non-numeric)
...
ggplot(data = got_top100, aes(label = word, size = n)) +
  ...
  scale_color_gradientn(colors = c("royalblue","orange","violet")) +
  theme_minimal()
```
...
#the afinn lexicon assigns a score of 2 to words that are somewhat positive in sentiment, but not as strongly positive as words with higher scores (e.g., 3, 4, or 5).
...
```{r nrc-confidence}
stone <- get_sentiments(lexicon = "nrc") %>% 
  filter(word == "stone")
...
In Game of Thrones, commonly occurring meaningful words include terms like “stone,” “time,” “watch,” “lord,” and “dark,” each appearing within different sentiment categories. I would expect the overall volume of emotional language to be dominated by negative emotions, as the book centers on conflict and frequent death.
Consequently, emotions such as anger and fear are likely to be prominent. For example, “stone” is associated with anger, while “watch” associated with fear, these words are therefor similarly ambiguous to  "confidence" in the ipcc report. 
